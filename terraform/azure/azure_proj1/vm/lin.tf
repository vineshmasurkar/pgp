...
## Create Ubuntu-20.04 LTS virtual machine - App Server
...
#   }
  ...
  network_interface_ids = [var.pub_nic_id]
...
## Attach azure-manged data disk to virtual machine - App Server
...
## Create Ubuntu-20.04 LTS virtual machine - DB Server
resource "azurerm_linux_virtual_machine" "db_server_vm" {
  name                  = "${var.proj}-db-server-vm"
  computer_name         = "${var.proj}"
  admin_username        = "azureuser"
#   admin_password        = "CorePassword123"
  admin_ssh_key {
  ...
  location              = var.rg_config.location
  resource_group_name   = var.rg_config.name
  network_interface_ids = [var.pvt_nic_id]
  size                  = "Standard_B1s" ## 1 vcpu, 1 GiB memory
  ## The free account includes access to three types of VMs for free
  ##  — the B1S, B2pts v2 (ARM-based), and B2ats v2 (AMD-based) 
  ##  burstable VMs that are usable for up to 750 hours per month.
  ...
  # custom_data           = file("custom-data.ps1") ## TO BE TESTED
  os_disk {
    name                 = "${var.proj}_db_server_os_disk"
    caching              = "ReadWrite"
    storage_account_type = "Standard_LRS"
  }
  source_image_reference {
...
## Attach azure-manged data disk to virtual machine - DB Server
resource "azurerm_managed_disk" "db_server_data_disk" {
  name                  = "${var.proj}_db_server_data_disk"
  location              = var.rg_config.location
  resource_group_name   = var.rg_config.name
  storage_account_type  = "Standard_LRS"
  create_option         = "Empty"
...
resource "azurerm_virtual_machine_data_disk_attachment" "db_server_data_disk_attach" {
  managed_disk_id    = azurerm_managed_disk.db_server_data_disk.id
  virtual_machine_id = azurerm_linux_virtual_machine.db_server_vm.id
  lun                ="10"
  caching            = "ReadWrite"
}
